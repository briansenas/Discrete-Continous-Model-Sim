...
void  resolver (double _t0, double _tf, double * _res ){
    ...
    int main(int argc, char** argv){
        ...
        if(argc<=5){
            cerr << "[ERROR]: Expected 3 parameters, [Days] [fish] [sharks] [pescado] [frecuencia] ([__F])" << endl;;
            exit(-1);
        }
        _tfin = atof(argv[1]);
        __x[0] = atof(argv[2]);
        ...
        stringstream oss;
        oss << std::fixed << setprecision(0) << path << "river_results/pescar-"<<__F << "-" << frecuencia << "-" << pescado << ".txt";
